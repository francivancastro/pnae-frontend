...
export default function AlterarProdutor() {
  ...
  const {id} = useParams();
  const defaultProdutor = useLocation().state;
  ...
  useEffect(() => {
    async function getProdutor() {
      try {
        setNome(defaultProdutor.name);
        setCpf(cpfMask(defaultProdutor.cpf));
        setDap(defaultProdutor.dap);
        setSexo(defaultProdutor.gender);
        setCelular(celularMask(defaultProdutor.cell_phone));
        setTelefone(telefoneMask(defaultProdutor.lindline));
        setCep(cepMask(defaultProdutor.address.cep));
        setStreet(defaultProdutor.address.street)
        setBairro(defaultProdutor.address.district);
        setNumero(defaultProdutor.address.number);
        setEstado(defaultProdutor.address.state);
        setMunicipio(defaultProdutor.address.city);
        setDelivery_fee(defaultProdutor.delivery_fee)
       
        if (defaultProdutor.image){
          const buff = new Buffer(defaultProdutor.image.data, 'base64')
        
          setImage(buff);
        }

        if(defaultProdutor.gender === 'm') setMasculino(true);
        if(defaultProdutor.gender === 'f') setFeminino(true);
    ...
    getProdutor();
  }, [defaultProdutor.address.cep, defaultProdutor.address.city, defaultProdutor.address.district, defaultProdutor.address.number, defaultProdutor.address.state, defaultProdutor.address.street, defaultProdutor.cell_phone, defaultProdutor.cpf, defaultProdutor.dap, defaultProdutor.delivery_fee, defaultProdutor.gender, defaultProdutor.image, defaultProdutor.lindline, defaultProdutor.name, id]);
  ...
  async function handleSubmit() {
    try {
      ...
      setLoading(true);
      await apiv2.put(`/producers/${id}`, body, {
      ...
      if(error instanceof yup.ValidationError){
        ...
        alert('Erro ao atualizar produtor, possiveis erros: cpf e dap são unicos, verifique se já não exista cadastro deles ou verifique sua conexão.')
      }
    } finally {
      setLoading(false);
    }
         ...
         <div className="inputContent">
          ...
          <div className="inputContent">
            ...
            <select id="estado" onChange={input => setEstado(input.target.value)}>
              {!estado && <option disabled selected>Escolha seu estado</option>}
              {states.map(option => {
                return <option value={option.uf} selected={option.uf === estado}>{option.name} - {option.uf}</option>
              })}
            </select>
          ...
          <div className="inputContent">
            ...
            <select disabled={!estado} id="municipio" onChange={input => setMunicipio(input.target.value)}>
              {!municipio && <option disabled selected>Escolha seu município</option>}
              {estado && states[states.findIndex(state => state.uf === estado)].cities.map(option => {
                return <option value={option.name} selected={option.name === municipio}>{option.name}</option>
              })}
            </select>
