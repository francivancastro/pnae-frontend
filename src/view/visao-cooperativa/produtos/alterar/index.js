...
export default function AlterarProduto() {
  ...
  useEffect(() => {
    ...
    getProduto();
    loadCategories();
  }, [defaultProduto.category, defaultProduto.description, defaultProduto.image, defaultProduto.name, defaultProduto.price, defaultProduto.qty, defaultProduto.unity]);
  ...
  async function handleSubmit() {
    try{
      ...
      if(error instanceof yup.ValidationError){
        ...
        alert('Erro ao atualizar produto, verifique sua conexão e tente novamente.')
      }
    } finally {
      setLoading(false);
    }
  ...
  function handleBackList(e) {
    ...
    if (window.confirm('Você tem certeza que deseja cancelar este cadastro?')){
        history.replace('/cooperativa/listagem/produtos');
    }
  }
